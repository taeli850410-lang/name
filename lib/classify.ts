...
/**
 * '임대'가 집이 아니라 전산 자원·장비를 빌리는 뜻일 때. 기관 이름(ORG_NAME)처럼 지우고 셉니다 —
 * 통째로 막으면 `GPU 임대료`와 `아파트`가 같이 있는 제목까지 떨어집니다.
 *
 * 2026-09-19 에 한국경제TV 최근 업로드 46편을 이 필터에 넣어 보니 세 편이 통과했는데
 * 그중 둘이 이것이었습니다:
 *
 *   [🔴09/18 #당잠사] 美 증시 3대지수 상승ㅣ… 네비우스 "엔비디아 GPU 임대료 20% 인상"
 *   컴퓨팅 임대 '줄섰다'...결국 AI가 답인가｜앵커브리핑
 *
 * 앞엣것은 그날 중개사용 브리핑 영상 세 칸 중 하나를 실제로 차지하고 있었습니다.
 * 남은 한 편(`서울 정비사업 이주 가구 18만…전세난에 기름 붓나`)만 진짜였습니다.
 *
 * '데이터센터'도 넣었습니다. 데이터센터 임대차는 진짜 상업용 부동산 이야기일 수 있지만,
 * 이 브리핑을 읽는 곳은 주거용 중개사무소이고, 요즘 이 낱말이 걸리는 자리는 거의 다
 * AI 설비 투자 기사입니다.
 */
const FALSE_LEASE = /(?:GPU|CPU|서버|클라우드|컴퓨팅|데이터센터|장비|설비|차량|렌터카)[을를의]?\s*임대\S*/gi;

export function isStrongRealEstate(text: string): boolean {
  if (ORG_PR.test(text) || OFF_TOPIC.test(text) || WAR.test(text)) return false;
  return STRONG.test(text.replace(ORG_NAME, " ").replace(FALSE_LEASE, " "));
}
