 ...
 */
/**
 * 기관 이름은 뜻을 보고 고른 낱말이 아닙니다. '한국토지주택공사'에는 토지와 주택이,
 * '한국부동산원'에는 부동산이 들어 있어서 — 스포츠단 응원 영상도, 신입사원 채용 공고도,
 * 비전선포식도 부동산 영상으로 통과했습니다. 재기 전에 이름부터 지웁니다.
 *
 * 영상 제목에만 씁니다. 보도자료에서는 '국토교통부'가 발표 주체라 지우면 안 됩니다.
 */
const ORG_NAME = /한국토지주택공사|토지주택공사|서울주택도시개발공사|서울주택도시공사|경기주택도시공사|주택도시보증공사|한국주택금융공사|한국부동산원/g;

/**
 * `전세` 뒤의 `계`는 두 갈래입니다 — 전세'계약'은 부동산이고 '전세계'(世界)는 아닙니다.
 * "전세계 주식 시장 초긴장", "전세계 1위 반도체"가 부동산 영상으로 실렸습니다.
 * 그래서 전세 뒤에 계가 오면 약이 따라올 때만 셉니다.
 */
const STRONG =
  /주택|부동산|아파트|청약|전세(?!계(?!약))|월세|임대|재개발|재건축|정비사업|분양|택지|양도세|종부세|취득세|공시가격|실거래|집값|GTX|역세권|중개|규제지역|거래허가|토지|DSR|LTV|신도시|공급대책|주거|영끌|주담대|빌라|오피스텔|다세대|연립주택|단독주택|입주권|분양권|조합원|분담금|전월세|보증금|특별공급|청약통장|갭투자|전용면적|보유세|재산세|공시지가|아파텔|집단대출|중도금|입주자모집|모집공고/;

/**
 * 기관 채널이 올리는 홍보 영상 — 채용, 브이로그, 공모전, 선포식, 견학, 사옥 안내.
 * 주제어가 걸려도 부동산 소식이 아닙니다. 기관 이름을 하나씩 지우는 것보다 이 편이
 * 덜 새고 덜 늘어납니다(구로·노원·은평 … 센터 이름을 끝없이 적지 않아도 됩니다).
 */
const ORG_PR =
  /채용\s*공고|신입\s*(?:직원|사원)|인턴|브이로그|공모전|선포식|타운홀|워크숍|견학|서포터[스즈]|구독자|언박싱|실버버튼|찾아오시는\s*길|스포츠단|응원|사장님|입문교육|명랑운동회|민방위|크리에이터|이벤트|퀴즈|캠페인|시상식|기념|달성|사업소|(?:종합|지원|상담|안심|연구개발|주거복지)센터|대\.?공\.?개/;

/**
 * 부동산 낱말이 걸려도 부동산 소식이 아닌 것 — 인사청문회와 생중계.
 *
 * 2026-09-14 수집에 `[🔴생중계] 김성수 대법관 후보자 청문회…'처남 찬스 강남 …` 가 담겼습니다.
 * 후보자가 가진 강남 집이 쟁점이라 '아파트'가 제목에 있어서 통과했습니다. 그 앞 기준선에도
 * `5명으로 줄어든 장관 후보자…인사청문회`가 같은 이유로 있었습니다. 둘 다 인사 이야기지
 * 계약 앞둔 사람이 볼 것이 아닙니다.
 *
 * 좁게 잡았습니다. 국정감사·대정부질문은 거기서 진짜 주택 정책이 나오므로 넣지 않았습니다.
 * 생중계는 무엇이든 브리핑에 실을 것이 아닙니다 — 고객이 열 때는 이미 끝나 있습니다.
 */
...
/**
 * 낱말은 맞는데 뜻이 다른 것. 기관 이름(ORG_NAME)처럼 세기 전에 지웁니다.
 * 통째로 막지 않고 그 자리만 지우는 이유는, `GPU 임대료 오르는데 옆 아파트값은?` 같은
 * 제목이 '아파트'로는 통과해야 하기 때문입니다.
 *
 * 둘 다 실제로 브리핑에 실렸던 것입니다.
 *
 *   선택지   `[🔴속보] "이란 쓸어버리는 것도 선택지"…유엔총회 앞두고 폭주하는 트럼프`
 *            (2026-09-20 수집) — 안에 '택지'가 들어 있어서, 중동 전쟁 기사가 공급(supply)
 *            주제의 부동산 소식으로 담겼습니다. 이 제목에 부동산 낱말은 그것 하나뿐입니다.
 *
 *   GPU 임대  `[🔴09/18 #당잠사] 美 증시 3대지수 상승ㅣ… 네비우스 "엔비디아 GPU 임대료 20% 인상"`
 *            (2026-09-19 수집) — 그날 중개사용 브리핑 영상 세 칸 중 하나였습니다. 같은 날
 *            한국경제TV 업로드 46편을 돌려 보니 통과한 세 편 중 둘이 이 경우였습니다
 *            (나머지 하나가 `컴퓨팅 임대 '줄섰다'...결국 AI가 답인가`).
 *
 * '데이터센터 임대'도 넣었습니다. 데이터센터 임대차는 진짜 상업용 부동산 이야기일 수 있지만,
 * 이 브리핑을 읽는 곳은 주거용 중개사무소이고, 요즘 이 낱말이 걸리는 자리는 거의 다
 * AI 설비 투자 기사입니다.
 *
 * '전세계'(世界)는 여기 두지 않았습니다 — STRONG 안에서 '전세' 뒤에 '계'가 오면 '약'이
 * 따라올 때만 세는 식으로 이미 짜여 있습니다.
 */
const FALSE_MATCH = /선택지|(?:GPU|CPU|서버|클라우드|컴퓨팅|데이터센터|장비|설비|차량|렌터카)[을를의]?\s*임대\S*/gi;
